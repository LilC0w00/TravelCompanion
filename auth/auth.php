...
// Récupérer les paramètres de voyage s'ils existent
$personnes = htmlspecialchars($_GET['personnes'] ?? '');
$destination = htmlspecialchars($_GET['destination'] ?? '');
$logement = htmlspecialchars($_GET['logement'] ?? '');
...
<body>
  ...
  <main>
    <div class="form-container">
      ...
      <form method="POST" action="../auth/auth.php<?php echo !empty($_SERVER['QUERY_STRING']) ? '?' . $_SERVER['QUERY_STRING'] : ''; ?>">
        <div class="form-row">
        ...
        <!-- Champs cachés pour les paramètres de voyage -->
        <?php if (!empty($personnes)): ?>
          <input type="hidden" name="voyage_personnes" value="<?php echo htmlspecialchars($personnes); ?>">
        <?php endif; ?>
        <?php if (!empty($destination)): ?>
          <input type="hidden" name="voyage_destination" value="<?php echo htmlspecialchars($destination); ?>">
        <?php endif; ?>
        <?php if (!empty($logement)): ?>
          <input type="hidden" name="voyage_logement" value="<?php echo htmlspecialchars($logement); ?>">
        <?php endif; ?>
        <?php if (!empty($transport)): ?>
          <input type="hidden" name="voyage_transport" value="<?php echo htmlspecialchars($transport); ?>">
        <?php endif; ?>
        <?php if (!empty($date)): ?>
          <input type="hidden" name="voyage_date" value="<?php echo htmlspecialchars($date); ?>">
        <?php endif; ?>
        <?php if (!empty($filtre)): ?>
          <input type="hidden" name="voyage_filtre" value="<?php echo htmlspecialchars($filtre); ?>">
        <?php endif; ?>
        ...
        <div class="form-row">
          <div class="form-group" style="width: 100%;">
            <a href="../auth/login.php" class="connect-btn" style="display: block; text-align: center; text-decoration: none;">Déjà un compte ? Se connecter</a>
          </div>
        </div>
      </form>
    </div>
